...
global mydir "~/Dropbox/0iSquared/iSquared_WHO/PREM/Methods/4_DataAnalysis/Workshop/PREM_DM_Sampling_Workshop_Ghana/"
cd $mydir
...
local startdate 	 20231121 /*First date of the actual listing - in YYYYMMDD */ 
...
***** C.4. Assign facility id <<<<<<<<<<== MUST BE ADAPTED: MAKE IT CONSISTENT WITH THE LIME SURVEY FORM
	...
	gen facilityid=.
		...
		replace facilityid = 1306 if q100bsq003==1 /* change */
		replace facilityid = 1304 if q100bsq004==1
		replace facilityid = 1305 if q100bsq005==1
		replace facilityid = 1203 if q100bsq006==1 /* change */
...
import excel "$chartbookdir/Ghana_PREM_Pilot_Chartbook_WORKING.xlsx", sheet("Facility_sample") firstrow clear 
	...
	rename *, lower
		...
		target_sam~1205 int     %10.0g                target_sample_size_revised_1205
		--------------------------------------------------------------------------------------------------
		Sorted by: 
...
***** F.2 Sample by each facility
    ...
    foreach level of local levels {
		...
		keep if facilityid ==`level' 
			...
			/*EDIT 12/5/2023 STARTS*/
			*sum target_sample_size /*target SS in each facility*/ 
			sum target_sample_size_revised_1205 /*REVISED target SS in each facility*/ 
			/*EDIT 12/5/2023 STARTS*/
...
***** F.4 Export 		
	...
	*** Add 0 infront of the phone number (11/30/2023)
	gen zero = "0"
...
save "$listingdatadir/PREM_`country'_R`round'_SAMPLE.dta", replace	
	drop type
export excel using "$listingdatadir/PREM_`country'_R`round'_SAMPLE.xlsx", firstrow(variables) nolabel replace

**************************************************************
* F.A Sampling - NEW SECTION: select all who are listed (11/30/2023) 
**************************************************************
use "$listingdatadir/PREM_`country'_R`round'_SAMPLE.dta", clear
	...
	/*
		...
		.         tab district type, m row
				   ...
				   |          facility_type
		  district | District   Health Ce       CHPS |     Total
		-----------+---------------------------------+----------
		Mfantseman |       116        122         45 |       283 
				   |     40.99      43.11      15.90 |    100.00 
		-----------+---------------------------------+----------
		South Dayi |       120        115         47 |       282 
				   |     42.55      40.78      16.67 |    100.00 
		-----------+---------------------------------+----------
			 Total |       236        237         92 |       565 
				   |     41.77      41.95      16.28 |    100.00 
...
keep if listinginfocomplete==1
	...
	*tab district type, m row
	...
	District_n |          facility_type
		   ame | District   Health Ce       CHPS |     Total
	-----------+---------------------------------+----------
	Mfantseman |        90        101         78 |       269 
			   |     33.46      37.55      29.00 |    100.00 
	-----------+---------------------------------+----------
	South Dayi |       114         93         47 |       254 
			   |     44.88      36.61      18.50 |    100.00 
	-----------+---------------------------------+----------
		 Total |       204        194        125 |       523 
			   |     39.01      37.09      23.90 |    100.00 
...
*/
	...
	*By district and facility, number of listing progress in detail
	use temp.dta, clear		
		...
		/*EDIT 2023/12/5*/
		#delimit; 
		graph bar num_* target_sample_size target_sample_size_revised_1205 if district_name == "`lev'",  
			...
			legend( 
				...
				label(4 "Speak" "the language" )
				label(5 "Listed" )
				label(6 "Target SS" "(INITIAL)" )	
				label(7 "Target SS" "(REVISED 12/5)" )	
				)
			bar(1, color(navy*0.4)) 
			bar(2, color(navy*0.6)) 
			bar(3, color(navy*0.8)) 
...
GREAT JOB! END OF DATA CLEANING AND MANAGEMENT AND SAMPLING 
