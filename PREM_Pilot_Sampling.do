...
export excel using "$listingdatadir/PREM_`country'_R`round'_LIST.xlsx", replace
...
***** F.2 Sample by facility
    ...
    foreach level of local levels {
		...
		keep if facilityid ==`level' 
			...
			sum target
			return list
			sample `r(mean)', count by(facilityid)
...
export excel using "$listingdatadir/PREM_`country'_R`round'_SAMPLE.xlsx", replace
